...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="VRS Manager Version Check")
    parser.add_argument("--skip-timestamp", action="store_true",
                       help="Skip timestamp validation (used by CI with auto-generated versions)")
    ...
    # Skip if --skip-timestamp flag is used (CI auto-generates version)
    if args.skip_timestamp:
        print("⏭️  Timestamp check skipped (--skip-timestamp flag)")
        print("   Version is auto-generated by CI pipeline")
    else:
