print("- VRS Manager - Version : 1114 - By Neil Schmitt -")
print("- VRS Manager - Version : 1114 - By Neil Schmitt -")
print("- VRS Manager - Version : 1114 - By Neil Schmitt -")
print("- VRS Manager - Version : 1114 - By Neil Schmitt -")
import os
...
def build_lookups(df):
    """Build 3 lookup dictionaries for matching"""
    lookup_cw = {}  # (SequenceName, EventName)
    lookup_cg = {}  # (SequenceName, StrOrigin)
    lookup_es = {}  # (EventName, StrOrigin) - NEW: content signature
    ...
    for idx, row in enumerate(df.itertuples(index=False, name=None)):
        ...
        key_es = (row[col_idx[COL_EVENTNAME]], row[col_idx[COL_STRORIGIN]])  # NEW
        ...
        if key_es not in lookup_es:
            lookup_es[key_es] = row  # NEW
        
        if (idx + 1) % 500 == 0 or idx == total - 1:
            print_progress(idx + 1, total, "Building lookups")
    
    finalize_progress()
    return lookup_cw, lookup_cg, lookup_es  # Return 3 dicts
...
def compare_rows(df_curr, prev_lookup_cw, prev_lookup_cg, prev_lookup_es):
    ...
    for idx, curr_row in enumerate(df_curr.itertuples(index=False, name=None)):
        ...
        # Stage 2: EventName changed - match by (Sequence, StrOrigin)
...
def classify_working_change(curr_row, prev_row, prev_lookup_cg):
...
def classify_alllang_change(curr_row, prev_row, prev_lookup_cg):
    key_cg = (curr_row[COL_SEQUENCE], curr_row[COL_STRORIGIN])
...
def process_working_comparison(df_curr, prev_lookup_cw, prev_lookup_cg, prev_lookup_es):
    """Compare with 3-key system"""
    ...
    for idx, curr_row in df_curr.iterrows():
        ...
        key_cg = (curr_row[COL_SEQUENCE], curr_row[COL_STRORIGIN])
        key_es = (curr_row[COL_EVENTNAME], curr_row[COL_STRORIGIN])  # NEW
        ...
        # Stage 3: SequenceName changed - NEW
        elif key_es in prev_lookup_es:
            prev_row = prev_lookup_es[key_es]
...
def find_working_deleted_rows(df_prev, df_curr):
    ...
    curr_keys_es = set((row[COL_EVENTNAME], row[COL_STRORIGIN]) for _, row in df_curr.iterrows())  # NEW
...
def process_alllang_comparison(df_curr, lookup_kr, lookup_en, lookup_cn, 
                               lookup_cg_kr, lookup_es_kr,  # Added lookup_es_kr
                               has_kr, has_en, has_cn):
    """All language with 3-key system"""
    log("\n" + "="*70)
...
def process_master_file_update():
    ...
    try:
        ...
        # STEP 3: BUILD LOOKUPS (3-KEY SYSTEM)
        log("\nBuilding lookups with 3-key system...")
        ...
        for _, row in df_high.iterrows():
            ...
            if key_es not in source_high_lookup_es:
                source_high_lookup_es[key_es] = row.to_dict()  # NEW
        ...
        for _, row in df_low.iterrows():
            ...
            if key_es not in source_low_lookup_es:
                source_low_lookup_es[key_es] = row.to_dict()  # NEW
        ...
        for _, row in df_target.iterrows():
            ...
            if key_es not in target_lookup_es:
                target_lookup_es[key_es] = row.to_dict()  # NEW
        ...
        for idx, source_row in df_high.iterrows():
            ...
            key_es = (source_row[COL_EVENTNAME], source_row[COL_STRORIGIN])  # NEW
            ...
            # Determine change type using 3-key lookup
            target_row = None
        ...
        # STEP 6: FIND DELETED ROWS (3-KEY SYSTEM)
        log("\nFinding deleted rows with 3-key system...")
        deleted_rows = []
        
        # Build all current keys (3 types)
        ...
        source_all_keys_es = set(source_high_lookup_es.keys()) | set(source_low_lookup_es.keys())  # NEW
        ...
        for _, target_row in df_target.iterrows():
            ...
            key_es = (target_row[COL_EVENTNAME], target_row[COL_STRORIGIN])  # NEW
            
            # Only mark as deleted if ALL 3 keys are missing
            if (key_cw not in source_all_keys_cw) and \
               (key_cg not in source_all_keys_cg) and \
               (key_es not in source_all_keys_es):  # NEW
                output_row = {}
                for col in output_structure:
...
def process_raw_vrs_check():
    ...
    try:
        ...
        log("Building lookup dictionaries with 3-key system...")
        prev_lookup_cw, prev_lookup_cg, prev_lookup_es = build_lookups(df_prev)
        ...
        changes, previous_strorigins, changed_columns_map, counter = compare_rows(
            df_curr, prev_lookup_cw, prev_lookup_cg, prev_lookup_es
        )
...
def process_working_vrs_check():
    ...
    try:
        ...
        prev_lookup_cw, prev_lookup_cg, prev_lookup_es = build_working_lookups(df_prev, "PREVIOUS")
        
        df_result, counter = process_working_comparison(df_curr, prev_lookup_cw, prev_lookup_cg, prev_lookup_es)
...
def process_all_language_check():
    ...
    try:
        ...
        if has_kr:
            ...
            df_kr = normalize_dataframe_status(df_kr)
            log(f"  → {len(df_kr):,} rows")
            lookup_kr, lookup_cg_kr, lookup_es_kr = build_working_lookups(df_kr, "KR PREVIOUS")
        ...
        if has_en:
            ...
            df_en = normalize_dataframe_status(df_en)
            log(f"  → {len(df_en):,} rows")
            lookup_en, _, _ = build_working_lookups(df_en, "EN PREVIOUS")
        ...
        if has_cn:
            ...
            df_cn = normalize_dataframe_status(df_cn)
            log(f"  → {len(df_cn):,} rows")
            lookup_cn, _, _ = build_working_lookups(df_cn, "CN PREVIOUS")
        
        df_result, counter = process_alllang_comparison(
            df_curr, lookup_kr, lookup_en, lookup_cn, lookup_cg_kr, lookup_es_kr,
            has_kr, has_en, has_cn
        )
...
def create_gui():
    window = tk.Tk()
    window.title("VRS Manager by Neil Schmitt (ver. 1114)")
    ...
    footer_label = tk.Label(
        window,
        text="ver. 1114 | 3-Key System | SequenceName Change Detection",
        font=("Arial", 8),
        bg=bg_color,
        fg="#999999"
    )
