...
class WorkingProcessor(BaseProcessor):
    ...
    def process_data(self):
        ...
        try:
            # Build lookups
            self.prev_lookup_cw, self.prev_lookup_cg, self.prev_lookup_es, self.prev_lookup_cs = \
                build_working_lookups(self.df_prev, "PREVIOUS")

            # Process comparison and import
            self.df_result, self.counter = process_working_comparison(
                self.df_curr,
                self.prev_lookup_cw,
                self.prev_lookup_cg,
                self.prev_lookup_es,
            ...
            # Find deleted rows
            self.df_deleted = find_working_deleted_rows(self.df_prev, self.df_curr)
