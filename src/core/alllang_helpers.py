    ...
    for idx, curr_row in df_curr.iterrows():
        ...
        if has_kr:
            ...
            # LEVEL 2: 2-Key Matches
                ...
                changes = []
                if S != prev_kr[COL_SEQUENCE]:
                    changes.append("SequenceName")
                if E != prev_kr[COL_EVENTNAME]:
                ...
                if prev_kr:
                    ...
                    if C != prev_kr[COL_CASTINGKEY]:
                        changes.append("CastingKey")
                    change_type = "+".join(changes) + " Change" if (changes and contains_korean(O)) else "No Relevant Change"
                else:
