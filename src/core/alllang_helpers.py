...
from src.config import (
    ...
    COL_DIALOGVOICE, COL_SPEAKER_GROUPKEY, COL_STARTFRAME, COL_DESC,
    COL_DIALOGTYPE, COL_GROUP, CHAR_GROUP_COLS
)
...
from src.core.change_detection import detect_all_field_changes, detect_dict_field_changes
...
def process_alllang_comparison(df_curr, prev_lookup_se, prev_lookup_so, prev_lookup_sc,
    ...
    """
    Compare and import data for All Language process using 10-key system.

    Args:
        df_curr: Current merged DataFrame
        prev_lookup_se: KR Previous lookup for (Sequence, Event)
        prev_lookup_so: KR Previous lookup for (Sequence, StrOrigin)
        prev_lookup_sc: KR Previous lookup for (Sequence, CastingKey)
        prev_lookup_eo: KR Previous lookup for (Event, StrOrigin)
        prev_lookup_ec: KR Previous lookup for (Event, CastingKey)
        prev_lookup_oc: KR Previous lookup for (StrOrigin, CastingKey)
        prev_lookup_seo: KR Previous lookup for (Sequence, Event, StrOrigin)
        prev_lookup_sec: KR Previous lookup for (Sequence, Event, CastingKey)
        prev_lookup_soc: KR Previous lookup for (Sequence, StrOrigin, CastingKey)
        prev_lookup_eoc: KR Previous lookup for (Event, StrOrigin, CastingKey)
        lookup_en: English lookup dictionary (key: (SequenceName, EventName))
        lookup_cn: Chinese lookup dictionary (key: (SequenceName, EventName))
    ...
    for idx, curr_row in df_curr.iterrows():
        ...
        if has_kr:
            ...
            # ========================================
            # STEP 2: Pattern Matching - Use Universal Detection
            # ========================================

            # LEVEL 1: 3-Key Matches
            elif key_seo in prev_lookup_seo:
                prev_kr = prev_lookup_seo[key_seo]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)

            elif key_sec in prev_lookup_sec:
                prev_kr = prev_lookup_sec[key_sec]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)

            elif key_soc in prev_lookup_soc:
                prev_kr = prev_lookup_soc[key_soc]
                change_type = detect_dict_field_changes(curr_dict, prev_kr, require_korean=O)

            elif key_eoc in prev_lookup_eoc:
                prev_kr = prev_lookup_eoc[key_eoc]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)

            # LEVEL 2: 2-Key Matches
            elif key_se in prev_lookup_se:
                prev_kr = prev_lookup_se[key_se]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)

            elif key_oc in prev_lookup_oc:
                prev_kr = prev_lookup_oc[key_oc]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)

            elif key_ec in prev_lookup_ec:
                prev_kr = prev_lookup_ec[key_ec]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)

            elif key_sc in prev_lookup_sc:
                prev_kr = prev_lookup_sc[key_sc]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)

            elif key_so in prev_lookup_so:
                old_eventname = prev_lookup_so[key_so]
                prev_kr = prev_lookup_se.get((S, old_eventname))
                change_type = detect_dict_field_changes(curr_dict, prev_kr, require_korean=O)

            elif key_eo in prev_lookup_eo:
                prev_kr = prev_lookup_eo[key_eo]
                change_type = detect_dict_field_changes(curr_dict, prev_kr)
...
"""Helper functions for All Language process - TWO-PASS version."""

from src.config import COL_SEQUENCE, COL_EVENTNAME, COL_STRORIGIN, COL_CASTINGKEY, COL_TEXT, COL_STATUS, COL_FREEMEMO
